# Package metadata.
TITLE       := Dolby TrueHD Decoder
VERSION     := 1.01
TITLE_ID    := BREW03085
CONTENT_ID  := IV0000-BREW03085_00-DOLBYTRUEHDDEC00

# Libraries linked into the ELF.
LIBS        := -lc -lkernel -lc++ -lSceAudioOut -lSceUserService -lSceSysmodule -lSceAudiodec 
...
eboot.bin: $(INTDIR) $(OBJS)
	...
	$(TOOLCHAIN)/bin/$(CDIR)/create-fself -in=$(INTDIR)/$(PROJDIR).elf -out=$(INTDIR)/$(PROJDIR).oelf --eboot "eboot.bin" --paid 0x3800000000000011 --authinfo  
